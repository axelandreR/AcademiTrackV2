...
import { DAYS_OF_WEEK, getTimeSlots, TIME_START, COLORS, CONTRACT_HOURS_TC, getShortLabel } from '../constants';
import { 
  Clock, MapPin, 
...
const ScheduleGrid: React.FC<ScheduleGridProps> = ({ 
  ...
  const stats = useMemo(() => {
    ...
    datesOfWeek.forEach(day => {
      // REGLA 1: Ignorar días después del 28/06 en el cálculo de carga semanal
      if (day.date > SEMESTER_END_DATE) return;

      const fileDaySessions = academicSchedulesInFile.filter(s => isScheduleActiveOnDate(s, day.date, day.key));
      fileDaySessions.forEach(s => fileLoadHours += s.weeklyHours);
      ...
      dayTasksInCalendar.forEach(s => {
        ...
        if (!s.isAdministrative) {
          if (isAutoestudio) asyncTotalMin += dur; else syncTotalMin += dur;
        } else {
          if (s.category === 'asincrona') asyncTotalMin += dur;
          else if (s.category === 'preparacion') prepTotalMin += dur;
          else if (s.category === 'coordinador') coordTotalMin += dur;
    ...
    const academicLoad = syncH + asyncH;
    
    const totalContractHours = academicLoad + (prepTotalMin / 60) + (coordTotalMin / 60) + (assignTotalMin / 60);
    
    // Si toda la semana es después del semestre, no hay discrepancia
    const isWeekOutOfSemester = datesOfWeek[0].date > SEMESTER_END_DATE;
    ...
    return {
      syncHours: syncH, asyncHours: asyncH, prepHours: prepTotalMin / 60, coordHours: coordTotalMin / 60,
      assignHours: assignTotalMin / 60, fileLoadHours, academicLoad, totalContractHours, targetLoadForWeek: fileLoadHours,
      hasAcademicDiscrepancy, hasContractDiscrepancy, isDeficit: !isHolidayInWeek && academicLoad < fileLoadHours - 0.01,
      isHolidayInWeek, isWeekOutOfSemester
    };
  }, [schedules, datesOfWeek, instructorType, holidays]);

  const auditObservations = useMemo(() => {
    const list: { date: Date; type: 'academic' | 'contractual'; meta: number; real: number }[] = [];
    if (!isInstructorView || !selectedFilterName) return list;
    const isTC = instructorType === 'TC';
    
    const firstDate = schedules.length > 0 ? new Date(Math.min(...schedules.map(s => s.startDate.getTime()))) : new Date();
    const getStart = (d: Date) => {
        const date = new Date(d); const day = date.getDay();
        const diff = date.getDate() - day + (day === 0 ? -6 : 1);
        date.setDate(diff); date.setHours(0,0,0,0); return date;
    };
    let scannerDate = getStart(firstDate);
    ...
    while (scannerDate <= SEMESTER_END_DATE) {
        ...
        for (let d = 0; d < 7; d++) {
            ...
            if (holidays.find(h => h.date.toDateString() === current.toDateString())) {
            ...
            calTasks.forEach(s => {
                ...
                if (!s.isAdministrative) { if (isAuto) wAsync += dur; else wSync += dur; } 
                else {
                  if (s.category === 'asincrona') wAsync += dur; else if (s.category === 'preparacion') wPC += dur;
                  else if (s.category === 'coordinador') wCoord += dur; else if (s.category === 'por_asignar') wAssign += dur;
                }
            });
        }

        // Si hay feriado, omitimos registrar observaciones para esta semana
        if (!hasHolidayInWeek) {
            const currentAcademicReal = wSync + wAsync;
            if (Math.abs(currentAcademicReal - wTarget) > 0.01) {
  ...
  useEffect(() => {
    if (isInstructorView) onDeficitStatusChange?.(stats.hasAcademicDiscrepancy || stats.hasContractDiscrepancy);
    else onDeficitStatusChange?.(false);
  }, [stats.hasAcademicDiscrepancy, stats.hasContractDiscrepancy, onDeficitStatusChange, isInstructorView]);
  ...
  const handleSlotClick = (dayKey: string, startTime: string) => {
    ...
    onAddAdministrativeTask(dayKey, startTime, requestedDuration, activeEditorTool, activeModality);
  };

  const gridBorderClass = (isInstructorView && (stats.hasAcademicDiscrepancy || stats.hasContractDiscrepancy)) 
          ...
          `}>
            <div className="flex flex-col md:flex-row xl:flex-col gap-6 md:gap-10">
              ...
              <div className="w-full md:w-[180px] xl:w-full pt-0 md:pt-0 xl:pt-6 xl:border-t border-white/10 flex flex-col gap-4">
                ...
                <div className={`flex flex-col p-1.5 rounded-2xl border border-white/5 w-full gap-2 ${activeEditorTool === 'por_asignar' || activeEditorTool === 'refrigerio' || activeEditorTool === 'coordinador' ? 'bg-slate-900 opacity-50' : 'bg-slate-800 shadow-inner'}`}>
                  <button disabled={activeEditorTool === 'por_asignar' || activeEditorTool === 'refrigerio' || activeEditorTool === 'coordinador'} onClick={() => setActiveModality('presencial')} className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase flex items-center justify-center space-x-2 ${ (activeModality === 'presencial') ? 'bg-slate-700 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}><MapPin size={12} /><span>Presencial</span></button>
                  <button disabled={activeEditorTool === 'por_asignar' || activeEditorTool === 'refrigerio' || activeEditorTool === 'coordinador'} onClick={() => setActiveModality('virtual')} className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase flex items-center justify-center space-x-2 ${ (activeModality === 'virtual') ? 'bg-slate-700 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}><Video size={12} /><span>Virtual</span></button>
                </div>
              </div>
            </div>
          </div>
                  ...
                  {datesOfWeek.map((day) => {
                    ...
                    return (
                      <div key={day.key} className={`relative h-full border-r border-slate-200 last:border-r-0 z-10 ${isOutOfPeriod ? 'bg-slate-50/30' : ''}`}>
                        ...
                        {schedules.filter(s => isScheduleActiveOnDate(s, day.date, day.key)).map((sched) => {
                          ...
                          return (
                            <div 
                              ...
                              className={`absolute left-[5px] right-[5px] p-2.5 rounded-2xl border-l-[6px] shadow-lg overflow-hidden transition-all hover:scale-[1.015] hover:z-[60] cursor-pointer flex flex-col group z-30 ${catColor}`} 
                            ...
                            >
                              <div className="absolute top-1.5 right-1.5 flex space-x-1 opacity-0 group-hover:opacity-100 z-[80] transition-opacity">
                                {sched.isAdministrative && onIndividualizeTask && sched.startDate.getTime() !== sched.endDate.getTime() && (
                                ...
                                {sched.isAdministrative && onDeleteRecord && (
                              ...
                              {!sched.isAdministrative ? (
                                <>
                                  ...
                                  <div className="flex flex-col flex-1 min-h-0 overflow-hidden mb-1">
                                    <h4 onClick={(e) => { e.stopPropagation(); onNavigate?.('Bloque', sched.block); }} className={`font-black leading-tight text-slate-900 whitespace-normal break-words hover:underline xl:text-sm lg:text-[11px] text-[10px]`}>{isAutoestudio ? `${sched.courseName} (AUTOESTUDIO)` : sched.courseName}</h4>
                                    {!isInstructorView && <div onClick={(e) => { e.stopPropagation(); onNavigate?.('Instructor', sched.instructor); }} className={`font-bold text-slate-500 mt-1 whitespace-normal break-words hover:underline hover:text-blue-600 transition-colors ${isLargeBlock ? 'text-[12px] mt-2' : 'text-[9px]'}`}>{sched.instructor}</div>}
          ...
          <div className="border-t-4 border-slate-900 bg-white shrink-0 shadow-[0_-15px_40px_rgba(0,0,0,0.15)] z-[100] relative">
            ...
            >
              <div className="flex items-center space-x-4">
                <div className={`p-2 rounded-lg ${(stats.hasAcademicDiscrepancy || stats.hasContractDiscrepancy) ? 'bg-rose-600 animate-pulse' : 'bg-blue-600'}`}>
                   {(stats.hasAcademicDiscrepancy || stats.hasContractDiscrepancy) ? <AlertTriangle size={16} /> : <CheckCircle size={16} />}
                </div>
                <div>
            ...
            `}>
               <div className="flex items-center space-x-5">
                  <div className={`hidden xl:block p-2.5 rounded-xl shadow-lg ${(stats.hasAcademicDiscrepancy || stats.hasContractDiscrepancy) ? 'bg-rose-600 animate-pulse' : 'bg-blue-600'}`}>
                    {(stats.hasAcademicDiscrepancy || stats.hasContractDiscrepancy) ? <AlertTriangle size={20} /> : <CheckCircle size={20} />}
                  </div>
                  <div className="flex items-center gap-4">
                    <div className="hidden xl:block">
                      <h3 className="text-sm font-black uppercase tracking-widest leading-none">Carga {instructorType}</h3>
                      <p className={`text-[10px] font-bold uppercase mt-1 ${(stats.hasAcademicDiscrepancy || stats.hasContractDiscrepancy) ? 'text-rose-300' : 'text-slate-400'}`}>
                        {stats.isWeekOutOfSemester ? 'Vigencia de Ciclo Finalizada' : stats.isHolidayInWeek ? 'Vigencia de Feriado (Auditoría Suspendida)' : (stats.hasAcademicDiscrepancy || stats.hasContractDiscrepancy) ? 'Alerta Detectada' : 'Estado Óptimo'}
                      </p>
                    </div>
               ...
               <div className="grid grid-cols-3 md:grid-cols-4 xl:flex xl:items-center flex-wrap gap-x-6 gap-y-4 justify-center md:justify-end mt-6 xl:mt-0 pt-6 xl:pt-0 border-t border-white/10 xl:border-none">
                  ...
                  {instructorType === 'TC' && (
                    <>
                      ...
                      <div className="flex flex-col items-center"><span className="text-[8px] font-black text-slate-500 uppercase mb-1">Coord</span><span className="text-base font-black text-slate-200">{(stats.coordHours + stats.assignHours).toFixed(2)}h</span></div>
                      <div className="h-8 w-px bg-white/10 hidden xl:block" />
              ...
              <div className="p-10 overflow-y-auto custom-scrollbar flex-1 space-y-10">
                <section>
                  <div className="flex items-center justify-between mb-6">
                    ...
                    {auditObservations.filter(o => o.type === 'academic').length === 0 ? <div className="flex items-center space-x-2 text-emerald-600 bg-emerald-50 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest"><CheckCircle size={14} /><span>Sin discrepancias</span></div> : <div className="flex items-center space-x-2 text-rose-600 bg-rose-50 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest"><AlertTriangle size={14} /><span>Observaciones</span></div>}
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {auditObservations.filter(o => o.type === 'academic').map((obs, i) => (
                      <div key={i} className="p-5 bg-rose-50 border border-rose-100 rounded-3xl flex items-center justify-between">
                        <div className="flex items-center space-x-4"><CalendarIcon size={20} className="text-rose-400" /><div><p className="text-[10px] font-black text-rose-800/60 uppercase">Semana {obs.date.toLocaleDateString('es-ES', { day: '2-digit', month: 'short' })}</p><p className="text-xs font-black text-rose-900">Obs. Carga Académica</p></div></div>
                        <div className="text-right"><p className="text-[10px] font-black text-rose-500 uppercase">Dif: {(obs.real - obs.meta).toFixed(2)}h</p><p className="text-[9px] font-bold text-rose-400 uppercase">Total: {obs.real.toFixed(2)}h / {obs.meta.toFixed(2)}h</p></div>
                      </div>
                    ))}
