...
const ReportsDashboard: React.FC<ReportsDashboardProps> = ({ schedules, instructors, holidays, onBack }) => {
  ...
  const semesterRange = useMemo(() => {
    ...
    return {
      start: getStartOfWeek(new Date(Math.min(...starts))),
  ...
  const auditData = useMemo(() => {
    ...
    return instructors.map(inst => {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        // Ignorar días post-semestre
        if (currentDate > SEMESTER_END_DATE) continue;

        const dayNames = ['DOMINGO', 'LUNES', 'MARTES', 'MIERCOLES', 'JUEVES', 'VIERNES', 'SABADO'];
        const dayName = dayNames[currentDate.getDay()];
        const holiday = holidays.find(h => h.date.toDateString() === currentDate.toDateString());
        if (holiday) hasHolidayS1 = true;

        const dayFileSessions = instAcademic.filter(s => s.days.includes(dayName) && currentDate >= s.startDate && currentDate <= s.endDate);
        dayFileSessions.forEach(s => metaCargaS1 += s.weeklyHours);
      ...
      while (scannerDate <= globalEnd && scannerDate <= SEMESTER_END_DATE) {
        ...
        const currentWeekEnd = new Date(scannerDate);
        currentWeekEnd.setDate(scannerDate.getDate() + 6);
        ...
        for (let i = 0; i < 7; i++) {
          ...
          instSchedules.filter(s => s.days.includes(dName) && d >= s.startDate && d <= s.endDate).forEach(s => {
            ...
            else if (isAuto || s.category === 'asincrona' || s.category === 'preparacion' || s.category === 'coordinador') wReal += dur;
          });
        }
        
        // REGLA: Ignorar discrepancias en semanas con feriado
        if (!hasHolidayInWeek && Math.abs(wReal - wMeta) > 0.01) {
          discrepancies.push({
            weekStart: new Date(scannerDate), weekEnd: new Date(currentWeekEnd),
      ...
      const deficitS1 = !hasHolidayS1 && (firstWeekStart <= SEMESTER_END_DATE) && (cargaRealS1 < metaCargaS1 - 0.01);
      
      let finalStatus: 'DEFICIT' | 'EXCESO' | 'OK' | 'NO_LOAD' = deficitS1 ? 'DEFICIT' : (cargaRealS1 > metaCargaS1 + 0.01 && !hasHolidayS1 ? 'EXCESO' : 'OK');
      if (metaCargaS1 === 0 && cargaRealS1 === 0) finalStatus = 'NO_LOAD';
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredAudit.map((row, idx) => (
                  <tr key={idx} className="hover:bg-slate-50/80 transition-colors group">
                     ...
                     <td className="px-8 py-5"><div className="flex items-center space-x-2"><Activity size={14} className="text-slate-300" /><span className={`text-sm font-black ${row.status === 'DEFICIT' ? 'text-rose-600' : row.status === 'EXCESO' ? 'text-amber-600' : row.status === 'NO_LOAD' ? 'text-amber-500' : 'text-emerald-600'}`}>{row.cargaReal.toFixed(2)}h</span></div>{row.hasHolidayS1 && <span className="ml-2 text-[8px] font-black bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded uppercase">Feriado</span>}</td>
                ...
                <div className="space-y-4">
                  {selectedAudit.discrepancies.map((d, idx) => (
                    <div key={idx} className="flex items-center justify-between p-5 bg-slate-50 border border-slate-100 rounded-3xl group hover:border-indigo-200 hover:bg-white transition-all shadow-sm">
                       <div className="flex items-center space-x-4">
                          <div className="p-3 bg-white rounded-xl shadow-sm text-slate-400 group-hover:text-indigo-600 transition-colors"><Calendar size={20} /></div>
                          <div><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Semana del Ciclo</p><p className="text-sm font-black text-slate-900">{d.weekStart.toLocaleDateString('es-ES', {day:'2-digit', month:'short'})} al {d.weekEnd.toLocaleDateString('es-ES', {day:'2-digit', month:'short'})}</p></div>
                       </div>
                       <div className="text-right">
                          <div className={`inline-flex items-center space-x-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${d.type === 'deficit' ? 'bg-rose-100 text-rose-700' : 'bg-amber-100 text-amber-700'}`}>
                             {d.type === 'deficit' ? <TrendingDown size={12} /> : <Activity size={12} />}
                             <span>{d.type === 'deficit' ? `-${d.diff.toFixed(2)}h` : `+${d.diff.toFixed(2)}h`}</span>
                          </div>
                          <p className="text-[9px] font-bold text-slate-400 mt-2 uppercase">Meta: {d.meta.toFixed(2)}h | Real: {d.real.toFixed(2)}h</p>
                       </div>
                    </div>
                  ))}
                </div>
