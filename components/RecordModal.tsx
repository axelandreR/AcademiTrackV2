...
const RecordModal: React.FC<RecordModalProps> = ({ isOpen, onClose, onSave, onNavigate, initialData, defaultInstructor, allSchedules = [], rooms = [] }) => {
  ...
  return (
    <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md">
      <div className="bg-white rounded-[24px] sm:rounded-[40px] shadow-2xl w-full max-w-4xl max-h-[95vh] overflow-hidden flex flex-col animate-in fade-in zoom-in duration-300 border border-white/20">

        {/* Cabezal */}
        <div className="px-5 sm:px-10 py-4 sm:py-6 border-b border-slate-100 flex items-center justify-between gap-3 bg-slate-50/80">
          <div className="flex items-center gap-3 sm:space-x-4 min-w-0">
            <div className={`p-2.5 sm:p-3 rounded-2xl text-white shadow-xl shrink-0 ${formData.isAdministrative ? 'bg-indigo-600 shadow-indigo-100' : 'bg-blue-600 shadow-blue-100'}`}>
              {formData.isAdministrative ? <Briefcase size={20} className="sm:hidden" /> : <BookOpen size={20} className="sm:hidden" />}
              {formData.isAdministrative ? <Briefcase size={24} className="hidden sm:block" /> : <BookOpen size={24} className="hidden sm:block" />}
            </div>
            <div className="min-w-0">
              <h3 className="text-base sm:text-xl font-black text-slate-900 tracking-tight leading-tight uppercase">
                {initialData ? (formData.isAdministrative ? 'Editar Tarea Administrativa' : 'Detalle de Clase') : 'Nuevo Registro de Horario'}
              </h3>
              <p className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] sm:tracking-[0.2em] mt-1 sm:mt-2 truncate">
        ...
        <form id="record-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-5 sm:p-10 space-y-6 sm:space-y-10 custom-scrollbar bg-white">
        ...
        {/* Footer */}
        <div className="px-5 sm:px-10 py-4 sm:py-8 border-t border-slate-100 flex justify-end gap-3 sm:space-x-4 bg-slate-50/80">
          <button type="button" onClick={onClose} className="px-4 sm:px-8 py-3 sm:py-3.5 text-xs font-black text-slate-500 hover:bg-slate-200 rounded-2xl transition-all uppercase tracking-widest">Cancelar</button>
          <button form="record-form" type="submit" className="px-5 sm:px-10 py-3 sm:py-3.5 bg-slate-900 text-white text-xs font-black rounded-2xl hover:bg-slate-800 shadow-2xl shadow-slate-200 transition-all flex items-center gap-2 sm:space-x-3 active:scale-95 uppercase tracking-widest whitespace-nowrap">
            <Save size={18} className="sm:hidden shrink-0" /><Save size={20} className="hidden sm:block shrink-0" /><span>Guardar Cambios</span>
          </button>
        </div>
      </div>
    </div>
