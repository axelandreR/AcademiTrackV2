...
const RecordModal: React.FC<RecordModalProps> = ({ isOpen, onClose, onSave, onNavigate, initialData, defaultInstructor }) => {
  ...
  const toggleDay = (day: string) => {
    if (!formData.isAdministrative && initialData) return; // Bloqueado para académicos
                  ...
                  {DAYS_OF_WEEK.map(day => {
                    ...
                    return (
                      <button 
                        ...
                        className={`px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border-2 ${isLocked ? 'cursor-not-allowed opacity-60' : ''} ${active ? 'bg-blue-600 border-blue-600 text-white shadow-lg shadow-blue-100' : 'bg-white border-slate-100 text-slate-400 hover:border-slate-300'}`}
                      >
                        {day.label}
                      </button>
                    );
             ...
             <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                   <div className="space-y-1.5">
                      ...
                      <input type="date" disabled={isLocked} className="w-full px-4 py-3 bg-slate-100 border-none rounded-2xl font-bold text-sm outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50" value={toInputDate(formData.startDate)} onChange={e => setFormData({ ...formData, startDate: new Date(e.target.value + 'T00:00:00') })} />
                   </div>
                   <div className="space-y-1.5">
                      <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Fecha Fin</label>
                      <input type="date" disabled={isLocked} className="w-full px-4 py-3 bg-slate-100 border-none rounded-2xl font-bold text-sm outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50" value={toInputDate(formData.endDate)} onChange={e => setFormData({ ...formData, endDate: new Date(e.target.value + 'T00:00:00') })} />
                   </div>
                </div>
             </div>
             <div className="space-y-4">
