    ...
    return (
        ...
        >
            ...
            {!sched.isAdministrative ? (
                <>
                    <div className="flex justify-between items-center gap-1 font-black uppercase text-[8px] opacity-80 mb-1 shrink-0">
                        <span role="link" tabIndex={0} aria-label={`Ver bloque ${sched.block}`} onClick={(e) => { e.stopPropagation(); onNavigate?.('Bloque', sched.block); }} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); e.stopPropagation(); onNavigate?.('Bloque', sched.block); } }} className="flex items-center min-w-0 hover:underline focus:outline-none focus:ring-2 focus:ring-blue-600 rounded"><Hash size={8} className="mr-1 shrink-0" /><span className="truncate">NRC: {sched.nrc} • {sched.block}</span></span>
                        <div className="flex items-center gap-1 shrink-0">
                            {sched.tempHEActive && <span className="whitespace-nowrap bg-amber-500 text-white px-1.5 py-0.5 rounded-full tracking-wide">HE TEMPORAL</span>}
                            {isExtra && <span className="whitespace-nowrap bg-slate-900 text-white px-1.5 py-0.5 rounded-full tracking-wide">EXTRAS</span>}
                            {sched.modality === 'presencial' ? <MapPin size={9} className="shrink-0" /> : <Video size={9} className="shrink-0" />}
                            <span className="whitespace-nowrap">{sched.modality?.toUpperCase()}</span>
                        </div>
                    </div>
                    <div className="flex flex-col flex-1 min-h-0 min-w-0 overflow-hidden mb-1">
                        <h4 onClick={(e) => { e.stopPropagation(); onNavigate?.('Bloque', sched.block); }} className={`font-black leading-tight text-slate-900 whitespace-normal break-words hover:underline cursor-pointer xl:text-sm lg:text-[11px] text-[10px]`}>
                            ...
                            {isInstructorView && sched.activity && (
                                <span className="ml-1.5 whitespace-nowrap inline-block text-[8px] font-black bg-slate-900/10 text-slate-600 px-1 py-0.5 rounded-md align-middle">{sched.activity}</span>
                            )}
                        </h4>
                        {!isInstructorView && (
                            <div className="flex flex-col mt-0.5 min-w-0">
                                ...
                                {sched.activity && (
                                    <div className={`mt-0.5 font-black text-indigo-600 uppercase tracking-tighter truncate ${isLargeBlock ? 'text-[10px]' : 'text-[8px]'}`}>
                                        TIPO: {sched.activity}
                                    </div>
                                )}
                            </div>
                ...
                <div className="flex flex-col h-full w-full min-w-0 items-center justify-center text-center px-1">
                    <h4 className="font-black leading-tight uppercase text-[10px] whitespace-normal break-words w-full">{getShortLabel(sched.courseName)} {sched.category !== 'coordinador' && sched.category !== 'refrigerio' ? `- ${sched.modality?.toUpperCase()}` : ''}</h4>
                    {sched.tempHEActive && <span className="text-[8px] font-black bg-amber-500 text-white px-1.5 py-0.5 rounded-full tracking-wide mt-0.5 whitespace-nowrap">HE TEMPORAL</span>}
                    {isExtra && <span className="text-[8px] font-black bg-slate-900/10 px-1.5 py-0.5 rounded-full tracking-wide mt-0.5 whitespace-nowrap">EXTRAS</span>}
                    <div className="font-black border-t border-black/5 w-full pt-1 mt-1 text-[10px] truncate">{sched.startTime} - {sched.endTime}</div>
                </div>
            )}
        </div>
    );
